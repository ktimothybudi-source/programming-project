...
#define TYPEWRITER_CPS       38.0f
#define DRAW_TEXT_BUF_BYTES  2048

static char s_drawLineBuf[DRAW_TEXT_BUF_BYTES];
...
static const DialogueLine DAY4_GENERATOR_FIXED[] = {
    { "Narrator", "The generator sat in the basement—Paul knew that from training.\nHe rushed down, every sound swallowed by the dark store until the stairwell." },
    { "Narrator", "Downstairs was so silent his own pulse seemed loud.\nSmall shapes tugged at the edge of his flashlight—hallucinations, he told himself,\nexhaustion playing tricks. He kept moving." },
    { "Narrator", "In front of the machine at last, his hands steadied.\nA loose cable, a tripped breaker—quick work. The hum rose; lights flickered on upstairs." },
    { "Paul", "Wow—for a second there I thought the killer sabotaged the light\nand would kill me right then and there." },
    { "Narrator", "Relief washed over him. He headed for the stairs—old wood,\neach step groaning under his weight as he climbed back toward the main floor." }
};

static const DialogueLine DAY4_FOOTSTEPS[] = {
    { "Narrator", "Midway up the stairs, heavy rapid footsteps suddenly rushed\ntoward the basement—fast, deliberate, closing in from above the stairwell." },
    { "Paul", "(Think. I don't have long.)" }
};
...
static const DialogueLine ENDING_1[] = {
    ...
    { "Narrator", "ENDING 1 — HIDING" }
};

static const DialogueLine ENDING_2[] = {
    { "Narrator", "Paul vaulted back down a step and sprinted toward the janitor nook.\nHis hands closed on a heavy metal mop handle—better than nothing." },
    { "Narrator", "He spun toward the stairs as a short, tanned figure filled the doorway—\ncurly mustache, machete glinting. Mike Hawk." },
    { "Paul", "GET BACK!" },
    { "Narrator", "Paul swung wild. The shaft cracked against the killer's arm;\nHawk snarled, staggered. Paul didn't wait—he bolted past, up the stairs,\nshoulder slamming the fire door. The store alarm screamed as he hit the pad." },
    { "Narrator", "Sirens answered in the distance. Paul collapsed behind the counter,\nshaking, alive. ENDING 2 — FIND A WEAPON" }
};

static const DialogueLine ENDING_4[] = {
    { "Paul", "WHO'S THERE? I'M ON THE PHONE WITH THE POLICE!" },
    { "Narrator", "His voice cracked through the stairwell, raw and huge.\nThe footsteps stopped—one second, two—then silence so complete\nhe heard water drip in a pipe." },
    { "Narrator", "Somewhere above, a door eased shut. The rush of steps faded,\nnot toward him but away—retreating into the stockroom dark." },
    { "Narrator", "Paul didn't move until his legs burned. When morning came,\nsecurity footage showed nothing—or no one admitted what it showed.\nHe clocked out one last time, trembling, breathing. ENDING 4 — SHOUT HELLO" }
};
...
static const DialogueScript SCRIPTS[] = {
    ...
    { ENDING_2, sizeof(ENDING_2)/sizeof(ENDING_2[0]) },
    { ENDING_4, sizeof(ENDING_4)/sizeof(ENDING_4[0]) },
...
void Dialogue_Update(DialogueSystem *dlg) {
    ...
    const char *text = dlg->current->lines[dlg->currentIndex].text;
    int textLen = text ? (int)strlen(text) : 0;
    float step = 1.0f / TYPEWRITER_CPS;

    if (IsKeyPressed(KEY_SPACE) || IsKeyPressed(KEY_ENTER)) {
        if (dlg->visibleChars < textLen) {
            dlg->visibleChars = textLen;
            dlg->typewriterAcc = 0.0f;
            return;
        }
        dlg->currentIndex++;
        dlg->visibleChars = 0;
    ...
    if (dlg->visibleChars < textLen) {
        ...
        dlg->typewriterAcc += dt;
        while (dlg->typewriterAcc >= step && dlg->visibleChars < textLen) {
            dlg->typewriterAcc -= step;
            dlg->visibleChars++;
            char c = text[dlg->visibleChars - 1];
            if (c == '.' || c == '!' || c == '?')
                dlg->typewriterAcc -= step * 4.0f;
            else if (c == '\n')
                dlg->typewriterAcc -= step * 2.5f;
        }
    }
}
...
void Dialogue_Draw(const DialogueSystem *dlg, int screenWidth, int screenHeight) {
    ...
    int totalLen = line->text ? (int)strlen(line->text) : 0;
    int n = dlg->visibleChars;
    if (n > totalLen) n = totalLen;
    if (n > DRAW_TEXT_BUF_BYTES - 1) n = DRAW_TEXT_BUF_BYTES - 1;
    if (line->text && n > 0) {
        memcpy(s_drawLineBuf, line->text, (size_t)n);
        s_drawLineBuf[n] = '\0';
        DrawText(s_drawLineBuf, textX, textY, 16, (Color){ 230, 230, 240, 255 });
    }

    const char *hint = (dlg->visibleChars < totalLen) ? "[Space / Enter] Skip" : "[Space / Enter] Continue";
